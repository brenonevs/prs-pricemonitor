...
from dafitiPriceBot.dafitiPriceBot import DafitiPriceBot
...
class MonitorDiscordBot(commands.Bot):
    ...
    async def on_message(self, message):
        ...
        if message.content in ['!stop', '!parar']:
            ...
            if self.dafiti_bot_instance:
                self.dafiti_bot_instance.stop_searching()
        ...
        if re.search(r"!pesquisar\(produto\s*=\s*.+?,\s*preço\s*=\s*.+?,\s*paginas\s*=\s*.+?,\s*site\s*=\s*.+?,\s*repetir\s*=\s*.+?\)", message.content):
            ...
            if "amazon" in site:
                ...
                self.dafiti_bot_instance = DafitiPriceBot(product, price, pages, message.author, loop, times)

                await self.dafiti_bot_instance.search_prices()
         ...
         # Comando para monitorar um link de listagem de produtos
            ...
            if "kabum" in site:
                ...
                await self.dafiti_bot_instance.search_link_prices(link)
            ...
            if "kabum" in site:
                ...
                self.dafiti_bot_instance = DafitiPriceBot(product, price, None, message.author, loop, times)

                await self.dafiti_bot_instance.search_specific_product(link_produto, preco_limite)
